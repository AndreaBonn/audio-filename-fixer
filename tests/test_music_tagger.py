...
class TestProcessFile:
    ...
    def test_processes_when_tags_complete_but_name_bad(self, tmp_path: Path, config: Config):
        """Tag completi + nome tipo YouTube → deve processare (name_ok è False)."""
        f = tmp_path / "yt-xk72hd9as_official.mp3"
        f.write_bytes(b"fake audio")
        complete_tags = {
            "title": "Creep",
            "artists": ["Radiohead"],
        ...
        with (
            ...
            patch("music_tagger.acoustid_lookup", return_value=None),
            patch("music_tagger.mb_search", return_value=None),
            patch("music_tagger.write_tags", return_value=True) as mock_write,
            patch("music_tagger.rename_file", return_value=str(f)),
            patch("music_tagger.file_checksum", return_value="abc"),
        ):
            result = process_file(str(f), {}, config)
        assert result is True
        mock_write.assert_called_once()

    def test_name_ok_rejects_official_keyword(self, tmp_path: Path, config: Config):
        """File con 'official' nel nome → name_ok False → non skip."""
        f = tmp_path / "Artist - Song official.mp3"
        f.write_bytes(b"fake audio")
        complete_tags = {
            "title": "Song",
            "artists": ["Artist"],
    ...
    def test_name_ok_rejects_long_numeric_id(self, tmp_path: Path, config: Config):
        """File con 6+ cifre consecutive �� name_ok False → non skip."""
        f = tmp_path / "Artist - Song 123456789.mp3"
        f.write_bytes(b"fake audio")
        complete_tags = {
            "title": "Song",
            "artists": ["Artist"],
    ...
    def test_name_ok_accepts_clean_format(self, tmp_path: Path, config: Config):
        """File nel formato 'Artist - Title.mp3' con tag completi → skip."""
        f = tmp_path / "Radiohead - Creep.mp3"
        f.write_bytes(b"fake audio")
        complete_tags = {
            "title": "Creep",
    ...
    def test_fallback_uses_existing_artists_when_parser_finds_none(
        self, tmp_path: Path, config: Config
    ):
        """Se il parser non trova artisti, usa quelli dai tag esistenti."""
        f = tmp_path / "just_a_title.mp3"
        f.write_bytes(b"fake audio")
        existing_tags = {
            "title": "",
        ...
        with (
            patch("music_tagger.already_processed", return_value=False),
            patch("music_tagger.read_existing_tags", return_value=existing_tags),
            patch("music_tagger.tags_look_complete", return_value=False),
            patch("music_tagger.acoustid_lookup", return_value=None),
            patch("music_tagger.mb_search", return_value=None),
            patch("music_tagger.write_tags", return_value=True) as mock_write,
        ...
        written_info = mock_write.call_args[0][1]
        assert written_info["artists"] == ["Known Artist"]
...
class TestScanDirectory:
    ...
    def test_scans_subdirectories_recursively(self, tmp_path: Path, config: Config):
        """os.walk deve trovare file audio in sottocartelle."""
        music_dir = tmp_path / "music"
        (music_dir / "rock" / "alternative").mkdir(parents=True)
        (music_dir / "rock" / "alternative" / "song.mp3").write_bytes(b"fake")
        (music_dir / "electronic" / "ambient").mkdir(parents=True)
        (music_dir / "electronic" / "ambient" / "track.flac").write_bytes(b"fake")
        config.music_dir = str(music_dir)
...
class TestMain:
    ...
    def test_dry_run_flag_sets_config(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        """--dry-run deve impostare config.dry_run = True."""
        music_dir = tmp_path / "music"
        music_dir.mkdir()
        monkeypatch.setattr(
            "sys.argv",
        ...
        def capture_scan(state, config):
            captured_config["dry_run"] = config.dry_run
            return (0, 0, 0)

        with (
            patch("music_tagger.setup_logging"),
            patch("music_tagger.load_state", return_value={}),
            patch("music_tagger.scan_directory", side_effect=capture_scan),
